<template>
	<view class="cartContainer">
		...
		<block v-else>
			<view class="cartList">
				<view class="cartItem" v-for="(shopItem,index) in cartList" :key="shopItem.id">
					...
					<text class='iconfont icon-xuanzhong' :class="shopItem.selected?'selected':''" @click="changeShopItemSelected(index)"></text>
					<view class="shopItem">
		...
		<view class="cartFooter">
			<text class='iconfont icon-xuanzhong' :class="isSelecetdAll?'selected':''"  @click="changeSelectedAll"></text>
...
<script>
	...
	export default {
		...
		methods:{
			...
			//修改购物车选中状态
			changeShopItemSelected(index){
				this.changeShopItemSelectedMutation({index})
			},
			//改变全选状态			
			changeSelectedAll(){
				// console.log(this.isSelecetdAll)
				this.changeShopItemAllSelected(this.isSelecetdAll)
			},
			...mapMutations(['changeShopItemCountMutation','changeShopItemSelectedMutation','changeShopItemAllSelected'])
		}
	}
</script>
